...
function Sidebar() {
  ...
  const tabList = [
    ...
    { name: "Customer", icon: <User2 />, link: "/customer" },
    { name: "Table", icon: <TableIcon />, link: "/table" },
    { name: "Cashier", icon: <CircleDollarSign />, link: "/cashier" },
    { name: "Orders", icon: <ShoppingBag />, link: "/orders" },
    { name: "Reports", icon: <History />, link: "/reports" },
  ];

  return (
    <div className="w-[100px] h-full bg-linen rounded-lg shadow-sm py-3 flex flex-col items-center justify-between ">
      <ul className="flex flex-col items-center gap-2 ">
        {tabList.map((tab) => (
          <li
            key={tab.name}
            className="relative w-[60px] p-2 cursor-pointer"
            onClick={() => setSelectedTab(tab.link)}
          >
            <Link to={tab.link}>
            ...
            {selectedTab === tab.link && (
              <motion.div
                layoutId="full-border-tabs"
                className="absolute inset-0 border-2 border-amber-sea rounded-md"
                initial={false}
                animate={{
      ...
      <ul className="flex flex-col items-center gap-4">
        <Link>
          <li className="border-2 bg-amber-sea/10 border-amber-sea w-[60px] p-2 rounded-md cursor-pointer">
        ...
        <li className="border-2 bg-amber-sea/10 border-amber-sea w-[60px] p-2 rounded-md s justify-center cursor-pointer">
          <Link>
            <LogOut />
          </Link>
        </li>
